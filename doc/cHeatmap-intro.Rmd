...
1.  Automatic or manual setting of the values of outliers in the input
    matrix so that the color scale of the heatmap is not dominated by
    those outliers
...
5.  Discrete color-value mapping for integer matrices containing few
    unique values
...
8.  Handling of edge cases:
    ...
    -   Inf and -Inf values in input matrix cause errors in
        `stats::dist()` for clustering, they are reset as NA.

    -   If missing values are present in the input matrix or row and
        column annotations, a legend for missing value is added.
...
differences among many cells barely visible, such as `nMat1` below.
...
Set `resetOutliers = T` to detect outliers in `mat1` and reset their
values, so that the difference in the values of `nMat1` can be easily
visualized. See [Reference
Manual](https://blueskypie.github.io/cHeatmap/reference/cHeatmap.html)
...
Instead of auto-detecting the outliers, use the `colMap` to set the
range. Values outside of the range are outliers. The default of `colMap`
is `c("green4"=NA, "white"=whiteValue, "red"=NA)`.
...
Also note that `Inf` is reset to `NA` to avoid error in clustering and a
legend for missing values is added automatically.
...
-   shorten the portion between 1 and 3 if majority of cluster 2 are in
    (3, 18)
...
Mark outliers as `*`; note by default `resetOutliers = TRUE` for numeric
matrices.
...
`nRowCluster` and `nColmCluster` specify the number of colors in the
...
If the number of unique values is greater than `intAsDiscreteCutoff`
whose default value is 6 in `cHeatmap` parameter settings, the color
...
Cluster the matrix by converting characters to their alphabetic orders,
i.e. `a .. f` to `1 .. 6`, and manually set the color-value mappings.
...
transformed to the range of (0,1), representing relative values across
visits.
